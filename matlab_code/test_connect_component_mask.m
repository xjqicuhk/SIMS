...
save_path_library = '../testdata/original_mask/';
mkdir(save_path_library);
%mkdir(save_path_library_test);
...
for c = 1:19
    ...
    for i = 1:size(list,2)
        ...
        library_mask = zeros(img_size_h,img_size_w,size(conn,1),'uint8');
        library_mask_pole = zeros(img_size_h,img_size_w,size(conn,1),'uint8');
       ...
       save([save_path_library_class,'/',list{1,i}(1:end-4),'.mat'],'library_mask','library_mask_pole');
