...
if(training_phase):
    ...
    mat_path = "../traindata/traindata_synthesis_1024_2048/traindata_mat/"
    label_path = "../traindata/traindata_synthesis_1024_2048/traindata_label/"
    label_gt_path = "../traindata/Label1024Full/"
    save_model_path = "../trainedmodels/synthesis/synthesis_1024"
    num_folder = 2
else:
    save_model_path = "../trainedmodels/synthesis/synthesis_1024"
    mat_path = "../testdata/synthesis/transform_order_1024/"
    save_result_folder = "../result/synthesis/transform_order_1024/"
...
def build_vgg19(input,reuse=False):
    ...
    net['input_reduce'] = tf.image.resize_bilinear(net['input'], (sp//2, sp), align_corners=True)
    net['conv1_1']=build_net('conv',net['input_reduce'],get_weight_bias(vgg_layers,0),name='vgg_conv1_1')
...
def recursive_generator(label,sp):
   # label_split,proposal_split = tf.split(label,[20,3],3)
    # encoder
   # proposal_tmp = tf.image.resize_bilinear(proposal, (sp//2, sp), align_corners=True)
    # 256 x 512
    label_tmp = tf.image.resize_bilinear(label, (sp//4, sp//2), align_corners=True)
   ...
   # 4 x 8
    ...
    #decoder
    downsampled_6 = tf.image.resize_bilinear(label, (sp//256, sp//128), align_corners=True)
   # 4 x 8
    ...
    net_6 = tf.image.resize_bilinear(net_6,(sp//128,sp//64), align_corners=True)
   # 8 x 16

    downsampled_5 = tf.image.resize_bilinear(label, (sp//128, sp//64), align_corners=True)
   #8 x 16
    ...
    net_5 = tf.image.resize_bilinear(net_5, (sp // 64, sp // 32), align_corners=True)
   # 16 x 32

    downsampled_4 = tf.image.resize_bilinear(label, (sp // 64, sp // 32), align_corners=True)
   # 16 x32
    ...
    net_4= tf.image.resize_bilinear(net_4, (sp // 32, sp // 16), align_corners=True)
   # 32 x 64


    downsampled_3 = tf.image.resize_bilinear(label, (sp // 32, sp // 16), align_corners=True)
    ...
    net_3 = tf.image.resize_bilinear(net_3, (sp // 16, sp // 8), align_corners=True)
   # 64 x 128

    downsampled_2 = tf.image.resize_bilinear(label, (sp // 16, sp // 8), align_corners=True)
    ...
    net_2 = tf.image.resize_bilinear(net_2, (sp // 8, sp // 4), align_corners=True)
   # 128 x 256

    downsampled_1 = tf.image.resize_bilinear(label, (sp // 8,sp //4), align_corners=True)
    ...
    net_1 = tf.image.resize_bilinear(net_1, (sp // 4, sp// 2), align_corners=True)
   # 256 x 512
    ...
    net = tf.image.resize_bilinear(net, (sp // 2, sp), align_corners=True)
    input_512 = tf.image.resize_bilinear(label, (sp // 2, sp))
   # 512 x 1024

    input_up = tf.concat([input_512,net],3)

    net_up = slim.repeat(input_up, 2, slim.conv2d, 128, [3, 3], rate=1, normalizer_fn=slim.layer_norm,
                        activation_fn=lrelu,
                        scope='g_decoder_512')

    net_up_reduce = slim.conv2d(net_up,3,[1,1],rate=1,activation_fn=None,scope='g_512'+'_conv100')
    #net_up_reduce = (net_up_reduce+1)*0.5
    net_up_1024 = tf.image.resize_bilinear(net_up_reduce,(sp, sp*2), align_corners = True)
    input = tf.concat([label, net_up_1024], 3)
   # 1024 x 2048
    '''
    net_feature = slim.repeat(input, 2, slim.conv2d, 32, [3, 3], rate=1, normalizer_fn=slim.layer_norm,
                        activation_fn=lrelu,
                        scope='g_decoder_1024')
    '''

    net_final=slim.conv2d(input,3,[1,1],rate=1,activation_fn=None,scope='g_1024'+'_conv100')
...
with tf.variable_scope(tf.get_variable_scope()) as scope:
  ...
  #  a=1
    ...
    p_semantic = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf.reshape(tf.image.resize_nearest_neighbor(label_gt, (crop_size_h//2,crop_size_w//2)), [batch_size * crop_size_h * crop_size_w//4, 20]),logits=tf.reshape(generator_semantic, [batch_size * crop_size_h * crop_size_w//4, 20])))
    ...
    p1=compute_error(vgg_real['conv1_2'],vgg_fake['conv1_2'],tf.image.resize_bilinear(label,(sp//2,sp//1), align_corners = True))/2.6
    p2=compute_error(vgg_real['conv2_2'],vgg_fake['conv2_2'],tf.image.resize_bilinear(label,(sp//4,sp//2), align_corners = True))/4.8
    p3=compute_error(vgg_real['conv3_2'],vgg_fake['conv3_2'],tf.image.resize_bilinear(label,(sp//8,sp//4),align_corners = True))/3.7
    p4=compute_error(vgg_real['conv4_2'],vgg_fake['conv4_2'],tf.image.resize_bilinear(label,(sp//16,sp//8),align_corners = True))/5.6
    p5=compute_error(vgg_real['conv5_2'],vgg_fake['conv5_2'],tf.image.resize_bilinear(label,(sp//32,sp//16),align_corners = True))*10/1.5
...
if ckpt:
   ...
   # uncomment the following line if finetuning from pretrained 512 x 1024 model
   '''
   saver_a = tf.train.Saver([v for v in tf.trainable_variables() if not "1024" in v.name])
   saver_a.restore(sess, ckpt.model_checkpoint_path)
   '''
   # comment this if finetuned from pretrained 512 x 1024 model
...
if training_phase:
    for epoch in range(1,101):
        ...
        while(global_ind<total_number):
            ...
            tmp_proposal = np.zeros((batch_size, crop_size_h, crop_size_w, 3), dtype=float)
            tmp_label_gt = np.zeros((batch_size, 19, crop_size_h, crop_size_w), dtype=float)
            z = np.zeros((batch_size, crop_size_h,crop_size_w,20), dtype = float)
            z_gt = np.zeros((batch_size, crop_size_h, crop_size_w,20), dtype = float)
        ...
        while (global_count < 99):
            ...
            for local_count in range(0,batch_size):
                ...
                try:
                    ...
                    print("cannot load"+"%08d.mat"%tmp_list[global_count])
            ...
            cv2.imwrite(save_path+ file_folder+"/%04d/%06d.png"%(epoch,cnt),np.uint8(output[0,:,:,:]))
    ...
    for ind in range(100001,100501):
        ...
        output=np.minimum(np.maximum(output, 0.0), 255.0)
        cv2.imwrite(save_result_path+"/%08d.png" %ind,
                    np.uint8(output[0, :, :, :]))
